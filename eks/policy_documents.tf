...
data "aws_iam_policy_document" "d_eks_load_balancer_controller" {
  statement {
    effect = "Allow"
...
# Load balancer assume role policy
data "aws_iam_policy_document" "r_eks_lb_assume_role" {
  statement {
    actions = ["sts:AssumeRoleWithWebIdentity"]
    effect  = "Allow"
    ...
    condition {
      test     = "StringEquals"
      variable = "${replace(aws_iam_openid_connect_provider.r_oidc_connect_provider.url, "https://", "")}:sub"
      values   = ["system:serviceaccount:kube-system:${var.lb_service_role}"]
    }
    condition {
      test     = "StringEquals"
      variable = "${replace(aws_iam_openid_connect_provider.r_oidc_connect_provider.url, "https://", "")}:aud"
